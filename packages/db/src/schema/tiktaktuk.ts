import { pgSchema, uuid, varchar, text, integer, timestamp, numeric, date, primaryKey, unique } from "drizzle-orm/pg-core";
...
export const accountRole = tiktaktukSchema.table("account_role", {
  roleId: uuid("role_id").notNull().references(() => role.roleId),
  userId: uuid("user_id").notNull().references(() => userAccount.userId)
}, (t) => [
...
export const customer = tiktaktukSchema.table("customer", {
  ...
  userId: uuid("user_id").notNull().unique().references(() => userAccount.userId)
});
...
export const seat = tiktaktukSchema.table("seat", {
  ...
  venueId: uuid("venue_id").notNull().references(() => venue.venueId, { onDelete: 'cascade' })
});
...
export const event = tiktaktukSchema.table("event", {
  ...
  venueId: uuid("venue_id").notNull().references(() => venue.venueId),
  organizerId: uuid("organizer_id").notNull().references(() => organizer.organizerId)
});
...
export const eventArtist = tiktaktukSchema.table("event_artist", {
  eventId: uuid("event_id").notNull().references(() => event.eventId),
  artistId: uuid("artist_id").notNull().references(() => artist.artistId),
...
export const ticketCategory = tiktaktukSchema.table("ticket_category", {
  ...
  teventId: uuid("tevent_id").notNull().references(() => event.eventId)
});
...
export const orders = tiktaktukSchema.table("orders", {
  ...
  customerId: uuid("customer_id").notNull().references(() => customer.customerId)
});
...
export const ticket = tiktaktukSchema.table("ticket", {
  ...
  tcategoryId: uuid("tcategory_id").notNull().references(() => ticketCategory.categoryId, { onDelete: 'cascade' }),
  torderId: uuid("torder_id").notNull().references(() => orders.orderId, { onDelete: 'cascade' })
});

export const hasRelationship = tiktaktukSchema.table("has_relationship", {
  seatId: uuid("seat_id").notNull().references(() => seat.seatId, { onDelete: 'cascade' }),
  ticketId: uuid("ticket_id").notNull().references(() => ticket.ticketId, { onDelete: 'cascade' })
}, (t) => [
  primaryKey({ columns: [t.seatId, t.ticketId] })
]);
...
export const orderPromotion = tiktaktukSchema.table("order_promotion", {
  orderPromotionId: uuid("order_promotion_id").primaryKey(),
  promotionId: uuid("promotion_id").notNull().references(() => promotion.promotionId),
  orderId: uuid("order_id").notNull().references(() => orders.orderId)
}, (t) => [
  unique("unique_order_promo").on(t.orderId, t.promotionId)
]);
